...
**Step 4 of 5 done 2026-09-26**: `Simulation` and the session format, so a multi-field rule is now a run rather than a pair of steppers. Storage is a `FieldStore` per declared field — a `HostGrid` and a `GpuGrid` of the field's cell type over the state's extents, because a field is a grid of one value per site and needs no second storage class. The field pairs swap with the state's in the same generation, `syncToHost` and `commitHost` move every field with the state (asymmetry there would have the next commit write stale field bytes back over live ones), and the span and texture vectors the steppers want are members refilled rather than built, so the step loop still allocates nothing. `setPath` is the one place the two authority rules meet and is tested by running a mixed-path trajectory against a pure one.

Installing a rule reallocates storage only when the field list differs by name or type, so a rule mutation — which never edits the list — leaves the fields holding what they held, while a rule that renamed or retyped a field starts it from zero rather than guessing what the old bytes mean. Allocation happens before anything is replaced, so a failure leaves the running rule and its fields untouched (AV-014).

The session gains an optional `fields` key: name, cell type and the buffer, in declaration order, using the state's own encodings. It is absent when the rule declares none, which is what makes an older session load unchanged. There is no `initial` for a field — a field starts at zero and the rule writes it from the state, so replay reconstructs it by doing the same arithmetic, and no journal event is needed because nothing outside the step loop writes one. A stored state whose fields are missing is partial rather than empty and is refused, as is one whose field list disagrees with its rule: a session is external data whatever wrote it, and a positional mismatch would read one field's bytes as another's. The sidecar carries the field buffers after the state's two, and whether it is used is decided per buffer group so that which files get one is unchanged for a rule without fields.

**Step 5 of 5 done 2026-09-26. F-031 is complete.** A multi-field fixture is in the equivalence sweep — two fields, one `u8` and one `f32`, read at this site and at a neighbour, plus a third nothing writes so the copy-forward is under the sweep too — and the sweep compares every field as well as the state, because a comparison that read the state alone would pass while the rule's own bookkeeping had diverged. `headless compare` does the same and names the field and the cell. The field storage the tests need is `tests/support/fields.hpp`, shared rather than copied.

Authoring is Lua, by D-023: `expr` is a table of constructors that nest into a tree, and the host flattens it into the arena so children precede parents without the author arranging it. Fields are named rather than indexed, read in two passes so a write may reference a field declared after it, and a `transition` that is an `expr` node is an expression rule while a function or an array is still a table rule — told apart by shape, since a script that built one and declared the other would be describing two different rules. Every expression rule is now authorable, not only a multi-field one; that came free, because a transition and a field write are the same thing. `LUA.md` documents the surface with examples that were run: the Life expression agrees cell-for-cell with `--rule B3/S23`, and a diffusing `f32` field reaches about 14 across 2303 distinct values with both paths bitwise identical.

The step found BUG-022, which step 4 had introduced: the editor's pad opens with the running rule, and a field rule reached `cpuStep` with no field buffers — an assert in debug and a read past nothing in Release. The pad refuses such a rule now.
