...
**Remaining:** the rest of the set this entry asks for — a Brian's Brain oscillator, a cyclic-CA seed, a hexagonal pattern and a 3D pattern for Bays' rules. None is transcribable from memory and all four would have to be constructed and then verified, which is a step of its own rather than a postscript to this one. The three Life patterns are published and transcribed; the Wireworld loop is constructed here and says so.
