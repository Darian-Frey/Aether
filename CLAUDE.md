...
Phase 6 — presentation and release. Nine deliverables, which is the largest phase in the roadmap and may want splitting; that has been flagged to the author twice and is undecided. Three clusters:

- **Patterns** (D-017): F-012 — the formats and the engine side of placement are done (2026-09-19: `sim/pattern`, SPEC §14, `Simulation::placePattern`/`extractPattern`, the `place` journal event). **F-012 is complete** as of 2026-09-21: formats, placement, the Patterns panel, the cursor preview, `--pattern`, and shift-drag selection writing a region back out. IMP-008 notes that the preview is ImGui rectangles and shows only an outline above 4096 cells. F-027's machinery and four verified patterns landed 2026-09-21; what remains of it is the Brian's Brain, cyclic, hexagonal and 3D entries of its set, none transcribable and all needing construction and verification. Then F-028 region seeding. Placement journals itself like any other grid mutation, or a session stops replaying (AV-016). A published Lenia or Life pattern is a *data* item: `rules/lenia.lua` notes that orbium needs its own seed, which is F-027's job rather than something to invent. `patterns/` is where `pathFor` puts a bare name, so the library and the save path already agree on where things live.
...
IMP-004 (the Seed button sits below the density sliders it is the point of) is worth doing with F-028 so the widgets move once.
...
Open: IMP-004 and IMP-008. Nothing open in BUGS. IMP-001, IMP-002, IMP-003, IMP-005, IMP-006 and IMP-007 are applied. Langton's loops is still untranscribed (F-010).
