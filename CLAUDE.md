...
Phases 1–3 complete (2026-09-11 to 2026-09-12); two Phase 4 features landed early (2026-09-14). The discrete core runs interactively in 2D (square and hexagonal) and 3D (volume raymarch, orbit, clip, slice painting) with both mutation controls, a lineage log with pin and rewind, and sessions that replay bit-identically across processes. Cells can be given an ageing tail and mutation can be grouped into blocks. The rest of Phase 4 (Lua and codegen) is not started.
...
0. ~~Cell life cycle (F-025) and correlated cell mutation (F-026).~~ Done 2026-09-14, out of order: neither needs Lua or codegen. `decay N` is capped at a 6-state tail on 2D Moore r=1 until step 4 or IMP-001 lands — IMP-001 (widened to a *counted set*) would make tails of any length free, and is now the cheaper of the two.
1. ~~`signature_literal` (IMP-002).~~ Done 2026-09-14. Wildcards and `rot`; mixable with count conditions; refused rather than lowered when the table is too big. Langton's loops is now writable but its 219 transitions are unwritten — a data item for step 3.
2. ~~Lua front end.~~ Done 2026-09-14. A transition may be an array or a function called per table entry; `expression` and `continuous` kinds are refused until a backend exists.
3. ~~Bundled rule library.~~ Done 2026-09-14. Thirteen rules; Langton's loops, the 1D rules and the 14-state cyclic CA are still out, each for a reason recorded in F-010.
3a. ~~Counted-set indexing (IMP-001).~~ Done 2026-09-14 as D-016. It lifted the table ceiling that had been capping ageing tails, Generations rules and the cyclic CA, so fewer rules now need step 4 at all.
...
Open: BUG-008 (default fill densities sum past 1 above nine states — a one-line fix, but which default is right is the author's call). F-024 screensaver mode is a Should in Phase 6. IMP-001, IMP-002 and IMP-003 are applied.
