...
- **Patterns** (D-017): F-012 — the formats and the engine side of placement are done (2026-09-19: `sim/pattern`, SPEC §14, `Simulation::placePattern`/`extractPattern`, the `place` journal event). Opening and placing are done (2026-09-21: the Patterns panel, the cursor preview, `--pattern`); what remains is the other direction, selecting a region and writing it out. IMP-008 notes that the preview is ImGui rectangles and shows only an outline above 4096 cells. Then F-027 the bundled library `patterns/` has been reserved for since the tree was created, and F-028 region seeding. Placement journals itself like any other grid mutation, or a session stops replaying (AV-016). A published Lenia or Life pattern is a *data* item: `rules/lenia.lua` notes that orbium needs its own seed, which is F-027's job rather than something to invent.
