...
Phases 1–4 complete (2026-09-11 to 2026-09-15); Phase 5 is in progress — the `f32` grid path and kernel authoring 2026-09-16, the continuous step on both paths 2026-09-17. Only rendering remains. The discrete core runs interactively in 2D (square and hexagonal) and 3D (volume raymarch, orbit, clip, slice painting) with both mutation controls, a lineage log with pin and rewind, and sessions that replay bit-identically across processes. Cells can be given an ageing tail and mutation can be grouped into blocks. Rules come from the DSL or from Lua, run through either the table or the codegen backend, and fourteen of them ship in `rules/`. An `f32` grid can be held, saved and loaded, Lua can author a continuous rule — a sampled kernel and a named growth function — and both paths step one, bitwise identically over 1000 generations on either GPU. It cannot be drawn yet, so a continuous rule is measured rather than watched.
...
5. Rendering: `f32` cells through the palette — all that is left of Phase 5. The 2D shader samples `usampler2D` and needs a float variant, `App` must give a grid the cell type its rule asks for (as `headless` now does), and a Lenia rule can then be bundled in `rules/`, which was held back so it would arrive usable rather than refusable.
6. ~~AV-015.~~ Done 2026-09-17, and better than expected: no narrowing was needed. The paths agree bitwise on Mesa and NVIDIA once generated float code is `precise` and never divides. That is two drivers on one machine — cross-*machine* is still untested, and is the risk that remains.
