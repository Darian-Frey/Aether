...
Phase 6 — presentation and release. Nine deliverables, which is the largest phase in the roadmap and may want splitting; that has been flagged to the author three times and is still undecided. The patterns cluster is done; two clusters remain:
...
- **The editor** (D-018): F-029 a host-side scratch pad and F-030 the cell inspector. The inspector reads the transition out of `sim::stepCell` rather than deriving its own — a second implementation would be free to drift while being the thing consulted when nobody can check it (AV-017). IMP-005 put the entry point there for exactly this.
...
The editor cluster is next: F-029 the scratch pad, then F-030 the inspector, which needs it.
...
Open: IMP-008. Nothing open in BUGS. IMP-001 to IMP-007 are all applied. Langton's loops is still untranscribed (F-010).
