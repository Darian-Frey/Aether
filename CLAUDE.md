...
Phases 1–4 complete (2026-09-11 to 2026-09-15); Phase 5 is built and half its acceptance met — the `f32` grid path and kernel authoring 2026-09-16, the continuous step on both paths, its rendering and BUG-011 2026-09-17. SmoothLife is what the acceptance still wants, and it needs two kernels per rule. The discrete core runs interactively in 2D (square and hexagonal) and 3D (volume raymarch, orbit, clip, slice painting) with both mutation controls, a lineage log with pin and rewind, and sessions that replay bit-identically across processes. Cells can be given an ageing tail and mutation can be grouped into blocks. Rules come from the DSL or from Lua, run through either the table or the codegen backend, and fourteen of them ship in `rules/`. A continuous rule can be authored in Lua, stepped on either path, and watched: `rules/lenia.lua` holds a self-organised field at 28% of the grid over 10,000 generations at 512², where the two drivers are bitwise identical and the CPU path agrees.
...
1. ~~`core/`: the `f32` grid path.~~ Done 2026-09-16. `HostGrid` kept its byte buffer and gained float views, which is what left the codec, the sidecar, the transfers and the VRAM arithmetic untouched. The session codec and the sidecar threshold now count bytes rather than cells — identical for `u8`, so no existing file changed — and `Simulation::create` refuses a grid whose cell type disagrees with its rule's. IMP-006 (the run-length codec expanding float data) was applied the same day: the writer now emits whichever inline encoding is shorter and names it, `rle` or the new `bytes`.
2. ~~`rule/`: kernel authoring.~~ Done 2026-09-16. Lua returns the profile as samples it computes itself, which is what made it cheap. The growth function is *named* rather than written out — `{form, mu, sigma}` lowered by `rule/growth` — because Lenia configurations are published that way and because `ExprOp` has no `exp`: a Gaussian growth is not expressible and deliberately stays that way, SPEC §6 forbidding reliance on built-ins of driver-defined precision. Polynomial and rectangular cover the acceptance. BUG-010 was found and fixed here: `Self` typed as an integer even in a growth expression. DSL syntax for kernels can follow whenever it is wanted; nothing depends on it.
3. ~~`sim/`: the continuous step.~~ CPU path done 2026-09-17; the GPU half needs step 4's generator and is listed there now. `stepCell` gained a continuous branch — convolve, grow, clamp — and D-020 settles what a kernel means, which SPEC had left open: distance sampling, the centre weight, normalisation to 1, and `dt` lowered into the growth expression rather than added to the IR. Reproducing a published Lenia glider needs its seed pattern as data (F-027): a uniform blob is not one, since at the orbium numbers the growth band is narrower than a single step.
4. ~~`rule/glsl` and the GPU step.~~ Done 2026-09-17. `aether_rule_f` from the growth expression; `continuous_step.comp` for the convolution, since the weights are the same arithmetic for every kernel. The equivalence fixture was red three times before it was green — see AV-015, which is now the most useful page in the register.
5. ~~Rendering: `f32` cells through the palette.~~ Done 2026-09-17. The palette pass is compiled twice from one source — a float grid is a `sampler2D` and a discrete one a `usampler2D`, and a sampler of the wrong type on a live unit is undefined even unread, so they cannot be branches of one program. `Palette::continuousRamp` is monotone, the hue cycling of `defaultFor` reading as a rainbow on a smooth field. `fillRandom`, `paintSpan` and `resetOutOfRangeStates` all walked the grid as bytes and now know better; the last of those fires on every continuous rule install.
6. ~~AV-015.~~ Done 2026-09-17. Generated float code is `precise` and never divides, and the oracle accumulates in `float`; the two drivers are then bitwise identical at 512² over 1000 generations. The determinism claim for `f32` did not need narrowing after all.

What remains of Phase 5 is SmoothLife, which the acceptance names and which `Kernel` cannot express: it wants an inner disc and an outer annulus, and a `Kernel` carries one profile. Two kernels per rule is an IR schema change and wants a decision of its own before anyone writes it.
...
Open: IMP-004 (the Seed button sits below the density sliders it is the point of). Nothing open in BUGS. IMP-005 and IMP-006 were applied 2026-09-16, so F-030's inspector now has the per-cell entry point it depends on. F-024 screensaver mode is a Should in Phase 6, along with the 1D space-time view the elementary rules are waiting for. Langton's loops is still untranscribed. IMP-001, IMP-002 and IMP-003 are applied.
