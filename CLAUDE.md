...
- **Patterns** (D-017): F-012 — the formats and the engine side of placement are done (2026-09-19: `sim/pattern`, SPEC §14, `Simulation::placePattern`/`extractPattern`, the `place` journal event). What remains is the interface: opening a file, positioning by cursor before committing, and writing a selection out. Then F-027 the bundled library `patterns/` has been reserved for since the tree was created, and F-028 region seeding. Placement journals itself like any other grid mutation, or a session stops replaying (AV-016). A published Lenia or Life pattern is a *data* item: `rules/lenia.lua` notes that orbium needs its own seed, which is F-027's job rather than something to invent.
...
Open: IMP-004. Nothing open in BUGS. IMP-001, IMP-002, IMP-003, IMP-005, IMP-006 and IMP-007 are applied. Langton's loops is still untranscribed (F-010).
