...
- **Patterns** (D-017): F-012 — the *formats* are done (2026-09-19, `sim/pattern`, SPEC §14); what remains is placement, which is interface work: putting a pattern into a running grid by cursor and journalling it, and exporting a selected region. Then F-027 the bundled library `patterns/` has been reserved for since the tree was created, and F-028 region seeding. Placement journals itself like any other grid mutation, or a session stops replaying (AV-016). A published Lenia or Life pattern is a *data* item: `rules/lenia.lua` notes that orbium needs its own seed, which is F-027's job rather than something to invent.
