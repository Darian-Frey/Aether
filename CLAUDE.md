...
- **Patterns** (D-017): F-012 import and export — Golly's extended RLE where it reaches, a native format for hexagonal, 3D and multi-state grids — F-027 the bundled library `patterns/` has been reserved for since the tree was created, and F-028 region seeding. Placement journals itself like any other grid mutation, or a session stops replaying (AV-016). A published Lenia or Life pattern is a *data* item: `rules/lenia.lua` notes that orbium needs its own seed, which is F-027's job rather than something to invent.
