...
Open: BUG-008 (default fill densities sum past 1 above nine states — a one-line fix, but which default is right is the author's call). F-024 screensaver mode is a Should in Phase 6, along with the 1D space-time view the elementary rules are waiting for. Langton's loops is still untranscribed. IMP-001, IMP-002 and IMP-003 are applied.
