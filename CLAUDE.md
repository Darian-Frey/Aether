...
Phases 1–4 complete (2026-09-11 to 2026-09-15); Phase 5 is in progress — the `f32` grid path and kernel authoring 2026-09-16, the continuous step on the CPU path 2026-09-17. The discrete core runs interactively in 2D (square and hexagonal) and 3D (volume raymarch, orbit, clip, slice painting) with both mutation controls, a lineage log with pin and rewind, and sessions that replay bit-identically across processes. Cells can be given an ageing tail and mutation can be grouped into blocks. Rules come from the DSL or from Lua, run through either the table or the codegen backend, and fourteen of them ship in `rules/`. An `f32` grid can be held, saved and loaded, Lua can author a continuous rule — a sampled kernel and a named growth function — and the CPU path steps one. The GPU path waits on the generator.
...
4. `rule/glsl`: `aether_rule_f(float self, float conv)` per SPEC §6, then the continuous half of `lut_step.comp` — the convolution loop over the resolved weights, which the stepper uploads like any other buffer — and the equivalence fixtures that prove the two paths agree. The float half of the expression interpreter is done and tested as of step 3 (`sim::evalGrowth`).
