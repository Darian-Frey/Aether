...
Phases 1–4 complete (2026-09-11 to 2026-09-15); Phase 5 is built but its acceptance is not met — the `f32` grid path and kernel authoring 2026-09-16, the continuous step on both paths and its rendering 2026-09-17, and BUG-011 open at 512². The discrete core runs interactively in 2D (square and hexagonal) and 3D (volume raymarch, orbit, clip, slice painting) with both mutation controls, a lineage log with pin and rewind, and sessions that replay bit-identically across processes. Cells can be given an ageing tail and mutation can be grouped into blocks. Rules come from the DSL or from Lua, run through either the table or the codegen backend, and fourteen of them ship in `rules/`. A continuous rule can be authored in Lua, stepped on either path, and watched: `rules/lenia.lua` holds a self-organised field at 28% of the grid indefinitely at 128² and 256², where the CPU path and both GPUs agree bitwise. At 512² it goes wrong on both GPUs and differently on each, which is BUG-011 and the reason Phase 5's acceptance is unmet.
...
6. **AV-015 and BUG-011 — what is left of Phase 5.** The arithmetic findings hold: generated float code must be `precise` and must not divide, and the oracle must not be more accurate than the shader. Up to 256² the CPU path and both GPUs agree bitwise. At 512² the wheels come off — NVIDIA grows four `0xFFFFFFFF` cells at the very start of the buffer by generation 5000 and is not reproducible against itself, Mesa empties the grid by generation 1000 — and BUG-011 has the evidence. Suspect the barrier or the download before suspecting the arithmetic: the bad cells are (0,0) to (3,0), which is the start of the transfer, and 256² is clean under every combination tried.
