...
**Effort:** small
**Description.** GL RAII objects must be destroyed while the context still exists, so `App::run` resets `sim_`, `renderer_`, `renderer3d_`, `previewGrid_` and now `spaceTime_` before `CloseWindow()`. Nothing enforces that list. A member that owns a GL handle and is not added to it is destroyed in `~App()` instead, after the context is gone, and the process segfaults on exit. That has now happened twice in consecutive features — `previewGrid_` in IMP-008 (BUG-016) and `spaceTime_` in F-005 — each time caught by running the thing rather than by reading the code, and each time in spite of the rule being written down in CLAUDE.md's pitfalls. (That sentence originally also claimed ATTACK_VECTORS carried it; checking while applying this, it does not, and neither does ARCHITECTURE. The point stands on the one document that did.) A rule that is followed by remembering it is a rule that will be forgotten; the interesting fact is not that it happened but that documenting it did not prevent the recurrence.
...
**As built.** Not the struct the proposal asked for. Grouping the members would have meant renaming every use of `sim_`, `renderer_` and the rest across `ui/` — the wide, shallow diff the trade-offs section flinched at — and it would still have been possible to declare a GL object outside the group.
What was done instead costs nothing at the use sites and gives a stronger guarantee: `App` owns the window itself, as its first member. C++ destroys members in reverse declaration order, so everything declared below `window_` is destroyed while the context is still open, and the teardown list is gone rather than centralised. There is no longer a list to forget to add to.
The one rule left is that `window_` stays above the members it protects, which is why it is the first thing in the class and says so.
Proved rather than argued, by doing the exact thing that crashed twice: a `core::GpuGrid` member added with no teardown line at all. Declared below `window_` the process exits 0; moved above it, the same member with the same absence of teardown segfaults. The declaration order is what does the work.
`rlImGuiShutdown` stays an explicit call at the end of `run()`, because it writes ImGui's layout through `iniPath_` — a member declared below the GL objects, and so destroyed before them. Moving it into the window's destructor would have swapped one ordering hazard for another.
